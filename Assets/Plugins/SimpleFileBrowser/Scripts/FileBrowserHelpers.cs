...
namespace SimpleFileBrowser
{
	...
	public static class FileBrowserHelpers
	{
		...
		public static bool IsPathChildOfAnother( string path, string parentPath )
		{
#if !UNITY_EDITOR && UNITY_ANDROID
			if( ShouldUseSAFForPath( path ) )
				return AJC.CallStatic<bool>( "IsSAFEntryChildOfAnother", Context, path, parentPath );
#endif
			path = Path.GetFullPath( path ).Replace( '\\', '/' );
			parentPath = Path.GetFullPath( parentPath ).Replace( '\\', '/' );
			...
			return path != parentPath && path.StartsWith( parentPath, System.StringComparison.OrdinalIgnoreCase );
		}
